...
def run_download_job(job_id, url, fmt, cookies=None, proxy=None):
    ...
    try:
        ...
        try:
            # Prefer a local cookies.txt file (project root) if present
            ...
            if os.path.exists(local_cookies):
                ...
                # Fallback: if INSTAGRAM_COOKIES env var is set, write it to a temp file
            ...
            logging.exception('Error while loading cookies from local file or INSTAGRAM_COOKIES')
        ...
        if cookiefile_path:
            # Sanitize cookie file into a strict Netscape cookie file that yt-dlp accepts.
            sanitized_cookie_path = None
            temp_sanitized_created = False
            try:
                src = cookiefile_path
                lines = []
                with open(src, 'r', encoding='utf-8', errors='ignore') as fh:
                    for raw in fh:
                        ...
                        if not raw:
                            continue
                        # Skip exporter metadata/header lines
                        if raw.lower().startswith('export format') or raw.lower().startswith('netscape') or raw.lower().startswith('table'):
                            continue
                        if raw.lower().startswith('domain') and 'name' in raw.lower() and 'value' in raw.lower():
                            continue
                        # Expect tab-separated fields; if not, split on whitespace
                        ...
                        name = parts[5]
                        value = '\t'.join(parts[6:])
                        # Unquote value and unescape common exporter escapes like \054 -> ,
                        if value.startswith('"') and value.endswith('"'):
                            value = value[1:-1]
                        value = value.replace('\\054', ',')
                        # Reconstruct a clean tab-separated Netscape line
                        clean_line = '\t'.join([domain, include_sub, path, secure, expiry, name, value])
                        lines.append(clean_line)

                if not lines:
                    logging.error('Cookie sanitization produced no cookie lines for source: %s', cookiefile_path)
                    with jobs_lock:
                        jobs[job_id]['status'] = 'error'
                        jobs[job_id]['error'] = 'Invalid cookie format — please re-export using the Get cookies.txt LOCALLY extension.'
                    return

                # write sanitized file to temp
                sf = tempfile.NamedTemporaryFile(delete=False, prefix='insta_sanitized_cookies_', suffix='.txt', dir=tempfile.gettempdir(), mode='w', encoding='utf-8')
                try:
                    sf.write('# Netscape HTTP Cookie File\n')
                    for l in lines:
                        sf.write(l + '\n')
                    sf.flush()
                finally:
                    sf.close()
                sanitized_cookie_path = sf.name
                temp_sanitized_created = True
                ydl_opts['cookiefile'] = sanitized_cookie_path
                logging.info('Using sanitized cookie file: %s (lines=%d)', sanitized_cookie_path, len(lines))
            except Exception:
                logging.exception('Failed to sanitize cookie file %s', cookiefile_path)
                # Fall back to original cookiefile; yt-dlp may still fail and will be handled below
        ...
        try:
            if 'temp_sanitized_created' in locals() and temp_sanitized_created and 'sanitized_cookie_path' in locals() and sanitized_cookie_path and os.path.exists(sanitized_cookie_path):
                os.remove(sanitized_cookie_path)
                logging.info('Removed temporary sanitized cookiefile %s', sanitized_cookie_path)
        except Exception:
            logging.exception('Failed to remove temporary sanitized cookie file')
